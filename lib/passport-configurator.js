...
PassportConfigurator.prototype.setupAuthHandler = function(options) {
    options = options || {};
...
PassportConfigurator.prototype.configureProvider = function(name, options) {
    ...
    var loginCallback = options.loginCallback || function(req, done) {
            return function(err, user, identity, token) {
                ...
                if(self.authHandler && user.metaId) {
                    self.authHandler(user, identity);
                }
            ...
            passport.use(name, new AuthStrategy(_.defaults({
                ...
                function(req, accessToken, refreshToken, profile, done) {
                    if (link) {
                        ...
                        profile.reqToken = req.query.state;
                        self.userIdentityModel.login(name, authScheme, profile,
                            {accessToken: accessToken, refreshToken: refreshToken},
                            options, loginCallback(req, done));
                    }
    ...
    if (authType === 'local') {
        ...
        self.app.get(authPath, function(req, res) {
            passport.authenticate(name, _.defaults({
                scope: scope,
                session: session,
                state: req.query.t
            }, options.authOptions))(req, res);
        });
    }
